  ...
  const fetchReservations = async () => {
    try {
      ...
      toast({
        variant: 'destructive',
        title: 'Error',
        description: message || 'No se pudieron cargar las reservaciones',
      })
    } finally {
      setLoading(false)
    }
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {reservations.length}{' '}
            {reservations.length === 1 ? 'reservación encontrada' : 'reservaciones encontradas'}
          </p>
        </div>
      ...
      {reservations.length === 0 && (
        <div className="rounded-lg border border-border bg-card p-8 text-center">
          ...
          <h2 className="text-lg font-semibold">No tienes reservaciones</h2>
          <p className="mt-2 text-sm text-muted-foreground">
      ...
      <div className="space-y-4">
        {reservations.map((reservation) => {
          const status = STATUS_CONFIG[reservation.estado as keyof typeof STATUS_CONFIG]
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div className="flex-1 space-y-3">
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge variant={status?.variant}>{status?.label ?? reservation.estado}</Badge>
                  ...
                  <div className="grid gap-3 text-sm md:grid-cols-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span>Total: Q{Number(reservation.precioTotal).toFixed(2)}</span>
                    </div>

                    {reservation.fechaCreacion && (
                      <div className="flex items-center gap-2">
                        <Calendar className="h-4 w-4 text-muted-foreground" />
                        <span>{new Date(reservation.fechaCreacion).toLocaleString('es-GT')}</span>
                      </div>
                    )}
                  </div>
                </div>
